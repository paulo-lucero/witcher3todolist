...
function exitNotes(evt) {//closing overlay notes menu
...
notesBody.addEventListener('click', exitNotes);

//testing overlay
const buttonNotes = document.getElementById('test-notes');

function testOpenNotes(evt) {
  notesBody.style.display = 'flex';
}

buttonNotes.addEventListener('click', testOpenNotes);
...
//   -use fix size for the notes content
//   -using "filter: blur()" seems to work also on non-image contents(e.g. text)
//    -enclose all elements in a div that will be blurred
//     -use 100% vw & vh, since viewport is basically the part of the document that is currently visible
...
//   -flebox: https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Flexible_Box_Layout#guides
